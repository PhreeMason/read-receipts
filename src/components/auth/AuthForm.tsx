...
export default function AuthForm({ type, onSubmit, isLoading }: AuthFormProps) {
    ...
    return (
        <View style={tw`gap-4`}>
            {type === 'register' && (
                <View style={tw`gap-2`}>
                    <Text style={tw`text-sm font-semibold text-gray-700`}>Username</Text>
                    <TextInput
                        style={tw.style(`p-4 border rounded-lg bg-white ${errors.username ? 'border-red-500' : 'border-gray-300'
                            }`)
                        }
