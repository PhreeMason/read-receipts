 ...
 */
export const separateBookData = (data: BookAndUserBookInsert, userId: string): {
    book: BookInsert,
    authors: AuthorInsert[],
    bookAuthors: BookAuthorInsert,
    ...
    // @ts-ignore
    const authorsData = data.metadata?.authors?.map((name: string) => ({
        name
    })) || [];

    // Create book-author relationships
    const bookAuthorsData = authorsData.map((author: { id: string }) => ({
        author_id: author.id
    }));
